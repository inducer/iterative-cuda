...
/*
 *  Compressed Sparse Column matrix (aka CSC)
 */
template <typename IndexType, typename ValueType>
struct csc_matrix : public matrix_shape<IndexType>
{
    typedef IndexType index_type;
    typedef ValueType value_type;

    IndexType * Ap;  //column pointer
    IndexType * Ai;  //row indices
    ValueType * Ax;  //nonzeros
};
...
template <typename IndexType, typename ValueType>
void delete_csc_matrix(csc_matrix<IndexType,ValueType>& csc, const memory_location loc){
    delete_array(csc.Ap, loc);  delete_array(csc.Ai, loc);   delete_array(csc.Ax, loc);
}
...
template <typename IndexType, typename ValueType>
void delete_host_matrix(csc_matrix<IndexType,ValueType>& csc){ delete_csc_matrix(csc, HOST_MEMORY); }
...
template <typename IndexType, typename ValueType>
void delete_device_matrix(csc_matrix<IndexType,ValueType>& csc){ delete_csc_matrix(csc, DEVICE_MEMORY); }
